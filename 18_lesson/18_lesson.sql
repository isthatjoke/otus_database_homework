...
-- Первичное заполнение таблицы good_sum_mart
...
CREATE OR REPLACE FUNCTION tf_sale_added()
  ...
  BEGIN
    ...
    SET sum_sale = (sum_sale + v_sale_amount)
...
CREATE OR REPLACE FUNCTION tf_sale_update()
  ...
  RAISE NOTICE 'старая сумма %', v_old_sale_amount;

  SELECT G.good_name, G.good_price * NEW.sales_qty
    INTO v_good_name, v_new_sale_amount
    FROM goods G
    WHERE G.goods_id = NEW.good_id;
  RAISE NOTICE 'новая сумма %, имя %', v_new_sale_amount, v_good_name;

  v_sum_diff := v_new_sale_amount - v_old_sale_amount;
  RAISE NOTICE 'разница сумм %', v_sum_diff;

  UPDATE good_sum_mart
    SET sum_sale = (sum_sale + v_sum_diff)
    WHERE good_name = v_good_name;
...
CREATE OR REPLACE FUNCTION tf_sale_canceled()
  ...
  BEGIN
    ...
    SELECT G.good_name, G.good_price * OLD.sales_qty
      INTO v_good_name, v_old_sale_amount
      FROM goods G
      WHERE G.goods_id = OLD.good_id;

    UPDATE good_sum_mart
      SET sum_sale = (sum_sale - v_old_sale_amount)
      WHERE good_name = v_good_name;
